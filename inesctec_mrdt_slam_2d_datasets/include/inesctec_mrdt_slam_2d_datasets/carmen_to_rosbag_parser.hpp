...
class CARMENToROSbagParser
{
 public:
  ...
  struct Param
  {
    std::string m_opts_yaml_config_file_path;  //!< YAML file path for the
  ...
  /**
   * @brief CARMEN to ROS parser stats (count number of messages correctly
   * parsed)
   */
  struct Stats
  {
    ...
    float m_max_range = -std::numeric_limits<float>::max();      //!< max range
    float m_max_range_2nd = -std::numeric_limits<float>::max();  //!< 2nd max
    float m_min_range = std::numeric_limits<float>::max();       //!< min range
    float m_min_range_2nd = std::numeric_limits<float>::max();   //!< 2nd min
 ...
 protected:
  ...
  rosbag::Bag m_bag_sort_;  //!< ROS bag (write order + header.seq by timestamp)

  bool m_tf_static_written_ = false;  //!< write laser->base tf only once
 ...
 protected:
  ...
  void addOdomMsg(double ts, double x, double y, double th, double v, double vn,
                  double w);

  void addLaserScanMsg(double ts, float angle_min, float angle_max,
                       float angle_inc, float range_min, float range_max,
                       const std::vector<float>& ranges,
                       const std::vector<float>& intensities, double odom_x,
                       double odom_y, double odom_th, double odom_v,
                       double odom_vn, double odom_w);
