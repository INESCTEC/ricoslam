#include "inesctec_mrdt_slam_distmap_2d_ros/slam_ros1_online.hpp"
...
SLAMROS1Online::SLAMROS1Online()
    : BaseType::SLAMROS1API(), m_tf2_sub_(m_tf2_buffer_)
{
  ...
  {
    m_pub_pose_ = std::make_unique<ros::Publisher>();
    *m_pub_pose_ =
        m_nh_priv_.advertise<geometry_msgs::PoseWithCovarianceStamped>(
            "pose", 10, false);
  }
...
void SLAMROS1Online::pubOdom(const std_msgs::Header& header)
{
...
void SLAMROS1Online::pubPose(const std_msgs::Header& header)
{
  ...
  if (m_param_.m_slam_pub_pose_msg)
  {
    ...
    pose_msg.header.seq = header.seq;
  ...
  {
    geometry_msgs::TransformStamped slam_tf_msg;
...
void SLAMROS1Online::pubExtOdom(
    const geometry_msgs::TransformStamped& ext_odom_tf)
{
...
void SLAMROS1Online::pubExtOdomTF(const nav_msgs::Odometry::ConstPtr& ext_odom)
{
