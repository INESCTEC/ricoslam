...
SLAMROS1Offline::SLAMROS1Offline(const ParamOffline& param)
    : BaseType::SLAMROS1API(), m_param_offline_(param)
{
  ros::Time::init();
  ...
  if (m_param_offline_.m_enable_log)
  {
    if (m_param_offline_.m_log_filename.empty())
    {
      throw std::runtime_error(
          "SLAMROS1Offline::SLAMROS1Offline | empty filename when log enabled");
    }
    ...
    try
    {
      ...
      log_file_odom = (dir / (stem + "_ricoslam_slam_odom" + ext)).string();
      log_file_pose = (dir / (stem + "_ricoslam_slam_pose" + ext)).string();

      ROS_INFO("[%s] log file  : %s", m_name_.c_str(), log_file_odom.c_str());
      ROS_INFO("[%s] log file  : %s", m_name_.c_str(), log_file_pose.c_str());
    }
    catch (const std::filesystem::filesystem_error& e)
    {
      throw std::runtime_error(
          "SLAMROS1Offline::SLAMROS1Offline | Error resolving paths for log "
          "files");
    }
    catch (const std::exception& e)
    {
      throw std::runtime_error(
          "SLAMROS1Offline::SLAMROS1Offline | Error when processing paths for "
          "log files");
    }
    catch (...)
    {
      throw std::runtime_error(
          "SLAMROS1Offline::SLAMROS1Offline | Unknown error when processing "
          "paths for log files");
    }

    system_utils::validateAndCreatePath(log_file_odom);
    system_utils::validateAndCreatePath(log_file_pose);

    try
    {
      m_log_file_odom_ = std::ofstream(log_file_odom);

      if (!m_log_file_odom_.is_open())
      {
        throw std::runtime_error("SLAMROS1Offline::SLAMROS1Offline | file (" +
                                 log_file_odom + ") for odom data not opened");
      }

      m_log_file_pose_ = std::ofstream(log_file_pose);

      if (!m_log_file_pose_.is_open())
      {
        throw std::runtime_error("SLAMROS1Offline::SLAMROS1Offline | file (" +
                                 log_file_pose + ") for pose data not opened");
      }
    }
    catch (const std::exception& e)
    {
      throw std::runtime_error(
          "SLAMROS1Offline::SLAMROS1Offline | error when opening the log "
          "files ( " +
...
SLAMROS1Offline::~SLAMROS1Offline()
{
  if (m_param_offline_.m_enable_log && m_log_file_odom_.is_open())
  {
    m_log_file_odom_.close();
  }

  if (m_param_offline_.m_enable_log && m_log_file_pose_.is_open())
  {
    m_log_file_pose_.close();
  }
...
void SLAMROS1Offline::pubOdom(const std_msgs::Header& header)
{
  if (!m_param_offline_.m_enable_log)
  {
    return;
  ...
  EstimateType odom_t = m_slam_->getTracker()->getRobotInOdom();
  ...
  try
  {
    m_log_file_odom_ << std::fixed << std::setprecision(9)
                     << header.stamp.toSec() << " " << odom_v.x() << " "
                     << odom_v.y() << " " << 0 << " " << odom_q.x() << " "
  ...
  {
    throw std::runtime_error(
        "SLAMROS1Offline::pubOdom | error when logging the robot data (" +
        std::string(e.what()) + ")");
  }
}

void SLAMROS1Offline::pubPose(const std_msgs::Header& header)
{
  if (!m_param_offline_.m_enable_log)
  {
    return;
  ...
  Eigen::Quaternionf pose_q(geometry::a2q(pose_v.z()));

  try
  {
    m_log_file_pose_ << std::fixed << std::setprecision(9)
                     << header.stamp.toSec() << " " << pose_v.x() << " "
                     << pose_v.y() << " " << 0 << " " << pose_q.x() << " "
  ...
  {
    throw std::runtime_error(
        "SLAMROS1Offline::pubPose | error when logging the robot data (" +
        std::string(e.what()) + ")");
  }
}

void SLAMROS1Offline::setupTerminal()
{
...
void SLAMROS1Offline::restoreTerminal()
{
  if (!m_terminal_modified_)
  {
    return;
...
void SLAMROS1Offline::printTime(const ros::Time& t,
...
char SLAMROS1Offline::readTerminalKey() const
{
  char c;
