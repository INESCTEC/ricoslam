...
SLAMROS2Offline::SLAMROS2Offline(const ParamOffline& param,
                                 const std::string& name,
                                 const rclcpp::NodeOptions& options)
    : BaseType::SLAMROS2API(name, options), m_param_offline_(param)
{
  rcl_ret_t ret =
  ...
  if (m_param_offline_.m_enable_log)
  {
    if (m_param_offline_.m_log_filename.empty())
    {
      throw std::runtime_error(
          "SLAMROS2Offline::SLAMROS2Offline | empty filename when log enabled");
    }
    ...
    {
      throw std::runtime_error(
          "SLAMROS2Offline::SLAMROS2Offline | Error resolving paths for log "
          "files");
    }
    catch (const std::exception& e)
    {
      throw std::runtime_error(
          "SLAMROS2Offline::SLAMROS2Offline | Error when processing paths for "
          "log files");
    }
    catch (...)
    {
      throw std::runtime_error(
          "SLAMROS2Offline::SLAMROS2Offline | Unknown error when processing "
          "paths for log files");
    }
    ...
    try
    {
      ...
      {
        throw std::runtime_error("SLAMROS2Offline::SLAMROS2Offline | file (" +
    ...
    {
      throw std::runtime_error(
          "SLAMROS2Offline::SLAMROS2Offline | error when opening the log "
          "files ( " +
...
SLAMROS2Offline::~SLAMROS2Offline()
{
...
void SLAMROS2Offline::pubOdom(const std_msgs::msg::Header& header)
{
  if (!m_param_offline_.m_enable_log)
  {
    return;
  ...
  {
    throw std::runtime_error(
        "SLAMROS2Offline::pubOdom | error when logging the robot data (" +
        std::string(e.what()) + ")");
  }
}

void SLAMROS2Offline::pubPose(const std_msgs::msg::Header& header)
{
  if (!m_param_offline_.m_enable_log)
  {
    return;
  ...
  {
    throw std::runtime_error(
        "SLAMROS2Offline::pubPose | error when logging the robot data (" +
        std::string(e.what()) + ")");
  }
}
