...
int main(int argc, char **argv)
{
  ...
  srrg_canvas = srrg_viewer->getCanvas(
      "slam_distmap_2d_slam_aligner_distance_map_point_plane_opengl");
  srrg_canvas->setName(
      "slam_distmap_2d_slam_aligner_distance_map_point_plane_opengl");
  ...
  std::thread run_tests(
      [&qapp, &srrg_viewer]()
      {
        ...
        using AlignerType = slam::AlignerDistanceMapPointPlane_<VariableType>;
        ...
        using NormalComputatorType =
            pcl::NormalComputatorNearestPoints1D_<MovingPointCloudType>;
        ...
        param.m_dbg_ogl_show_normals = true;
        ...
        std::cout << "AlignerDistanceMapPointPlane_<"
        ...
        while (srrg_viewer->isRunning())
        {
          ...
          float dr = geometry::degToRad(1.0f);
          ...
          switch (key)
          {
            ...
            case 'r':
            {
              test::getLaser2DPointCloudRandom(*pc_fixed, ang_res, fov,
                                               max_range, rng);
              test::getLaser2DPointCloudRandom(*pc_moving, ang_res, fov,
                                               max_range, rng);
              normal_computator.computeNormals(*pc_fixed);
              normal_computator.computeNormals(*pc_moving);
              break;
            ...
            case 'c':
            {
              test::getLaser2DPointCloudCircle(*pc_fixed, circle_radius,
                                               ang_res, fov, max_range, rng,
                                               noise_std);
              test::getLaser2DPointCloudCircle(*pc_moving, circle_radius,
                                               ang_res, fov, max_range, rng,
                                               noise_std);
              normal_computator.computeNormals(*pc_fixed);
              normal_computator.computeNormals(*pc_moving);
            ...
            case 'x':
            {
              test::getPointCloudSquare(*pc_fixed, square_side, square_side_pts,
                                        rng, noise_std);
              test::getPointCloudSquare(*pc_moving, square_side,
                                        square_side_pts, rng, noise_std);
              normal_computator.computeNormals(*pc_fixed);
              normal_computator.computeNormals(*pc_moving);
              break;
            }
            case 'l':
            {
              test::getPointCloudCorridor(*pc_fixed, corridor_width,
                                          corridor_length, square_side_pts, rng,
                                          false, noise_std);
              test::getPointCloudCorridor(*pc_moving, corridor_width,
                                          corridor_length, square_side_pts, rng,
                                          false, noise_std);
              normal_computator.computeNormals(*pc_fixed);
              normal_computator.computeNormals(*pc_moving);
