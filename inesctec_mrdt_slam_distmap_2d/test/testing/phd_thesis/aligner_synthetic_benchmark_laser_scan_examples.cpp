...
using VariableTypeDistMapp2pl =
    srrg2_solver::VariableSE2DistanceMapNanoStaticPointNormal2fVectorCloudRight;
// srrg2_solver::VariableSE2DistanceMapStaticWith2ndDerivPointNormal2fVectorCloudRight;
...
using MyTypes = data_structures::DerivedTypesPackExecutor_<
...
const char kLogFilenameAddInfo[] = "vox-n_subpx-n";

constexpr float kDistMapParamDistMapRes = .01f;
constexpr float kPointCloudNoiseStd = .00f;
...
struct BenchmarkConfig
{
  ...
  float m_circ_radius = 10.f;  //!< circle radius (m)
  float m_sqr_side = 10.f;     //!< square half-side (m)
  float m_corr_width = 5.f;    //!< corridor half-width (m)
  float m_corr_length = 50.f;  //!< corridor half-length (m)
...
{
  std::string m_name;
  std::function<void(PointCloudType&, PointCloudType&, const EstimateType&)>
      m_generate;
};
...
struct TrialResults
{
  ...
  double m_time_ms_dmap;
  double m_time_ms_align;
};
...
struct SceneResults
{
  ...
  double m_time_align_mean_ms;
  double m_time_align_std_ms;
  double m_time_align_min_ms;
  double m_time_align_max_ms;
  double m_time_dmap_mean_ms;
  double m_time_dmap_std_ms;
  double m_time_dmap_min_ms;
  double m_time_dmap_max_ms;
};
...
void saveTrialResultsCSV(const std::vector<AlignerSceneResults>& results,
                         ...
                         const BenchmarkConfig& cfg, AlignerTypeICPp2p& icp_p2p,
                         AlignerTypeICPp2pl& icp_p2pl,
                         AlignerTypeDistMapp2p& dmap_p2p,
                         AlignerTypeDistMapp2pl& dmap_p2pl);
                       ...
                       const EstimateType& gt_t, TrialResults& result)
{
  ...
  const auto tdmap_0 = std::chrono::high_resolution_clock::now();
  aligner.setTarget(fixed, EstimateType::Identity());
  const auto tdmap_1 = std::chrono::high_resolution_clock::now();

  aligner.setPriorEnabled(false);

  const auto talign_0 = std::chrono::high_resolution_clock::now();
  aligner.compute();
  const auto talign_1 = std::chrono::high_resolution_clock::now();
  ...
  result.m_time_ms_align =
      std::chrono::duration<double, std::milli>(talign_1 - talign_0).count();
  result.m_time_ms_dmap =
      std::chrono::duration<double, std::milli>(tdmap_1 - tdmap_0).count();
}
...
int main(int argc, char** argv)
{
  ...
  std::filesystem::create_directories(output_dir + "/examples");
...
SceneResults getSceneResults(const std::vector<TrialResults>& results)
{
  ...
  double sum_align_t = 0.0;
  double sum_align_t2 = 0.0;

  double sum_dmap_t = 0.0;
  double sum_dmap_t2 = 0.0;
  ...
  res.m_time_align_min_ms = std::numeric_limits<double>::max();
  res.m_time_align_max_ms = 0.0;
  res.m_time_dmap_min_ms = std::numeric_limits<double>::max();
  res.m_time_dmap_max_ms = 0.0;
  ...
  for (const auto& trial : results)
  {
    ...
    sum_align_t += trial.m_time_ms_align;
    sum_align_t2 += trial.m_time_ms_align * trial.m_time_ms_align;

    sum_dmap_t += trial.m_time_ms_dmap;
    sum_dmap_t2 += trial.m_time_ms_dmap * trial.m_time_ms_dmap;

    res.m_time_align_min_ms =
        std::min(res.m_time_align_min_ms, trial.m_time_ms_align);
    res.m_time_align_max_ms =
        std::max(res.m_time_align_max_ms, trial.m_time_ms_align);
    res.m_time_dmap_min_ms =
        std::min(res.m_time_dmap_min_ms, trial.m_time_ms_dmap);
    res.m_time_dmap_max_ms =
        std::max(res.m_time_dmap_max_ms, trial.m_time_ms_dmap);
  }
  ...
  res.m_time_align_mean_ms = sum_align_t / N;
  res.m_time_align_std_ms = std::sqrt(
      sum_align_t2 / N - res.m_time_align_mean_ms * res.m_time_align_mean_ms);

  res.m_time_dmap_mean_ms = sum_dmap_t / N;
  res.m_time_dmap_std_ms = std::sqrt(
      sum_dmap_t2 / N - res.m_time_dmap_mean_ms * res.m_time_dmap_mean_ms);
...
void saveTrialResultsCSV(const std::vector<AlignerSceneResults>& results,
                         ...
                         AlignerTypeDistMapp2pl& dmap_p2pl)
{
  int idx_variable_type_icp_p2p =
      MyTypes::findMatchingType(icp_p2p.getVariable());
  int idx_variable_type_icp_p2pl =
      MyTypes::findMatchingType(icp_p2pl.getVariable());
  int idx_variable_type_dmap_p2p =
      MyTypes::findMatchingType(dmap_p2p.getVariable());
  int idx_variable_type_dmap_p2pl =
      MyTypes::findMatchingType(dmap_p2pl.getVariable());

  auto get_aligner_m_level = [](const std::array<std::string, 20>& types_str,
                                int idx) -> std::string
  {
    std::string variable_type = types_str[idx];

    if (variable_type.find("Nano") != std::string::npos)
    {
      return std::string("m0");
    }
    else if (variable_type.find("StaticPoint") != std::string::npos)
    {
      return std::string("m1");
    }
    else if (variable_type.find("StaticWith") != std::string::npos)
    {
      return std::string("m2");
    }
    else
    {
      return std::string("m-error");
    }
  };

  std::string m_level_icp_p2p =
      get_aligner_m_level(my_types_str, idx_variable_type_icp_p2p);
  std::string m_level_icp_p2pl =
      get_aligner_m_level(my_types_str, idx_variable_type_icp_p2pl);
  std::string m_level_dmap_p2p =
      get_aligner_m_level(my_types_str, idx_variable_type_dmap_p2p);
  std::string m_level_dmap_p2pl =
      get_aligner_m_level(my_types_str, idx_variable_type_dmap_p2pl);

  for (const auto& asr : results)
  {
    std::string m_level;

    if (asr.m_aligner_type.compare("icp_p2p") == 0)
    {
      m_level = m_level_icp_p2p;
    }
    else if (asr.m_aligner_type.compare("icp_p2pl") == 0)
    {
      m_level = m_level_icp_p2pl;
    }
    else if (asr.m_aligner_type.compare("dmap_p2p") == 0)
    {
      m_level = m_level_dmap_p2p;
    }
    else if (asr.m_aligner_type.compare("dmap_p2pl") == 0)
    {
      m_level = m_level_dmap_p2pl;
    }
    else
    {
      m_level = "m-error";
    ...
    const std::string filepath = output_dir + "/" + asr.m_aligner_type + "_" +
                                 m_level + "_" + kLogFilenameAddInfo + "_" +
                                 asr.m_scene_name + ".csv";
    ...
    f << "trial,"
      ...
      << "time_ms_align,time_ms_dmap" << std::endl;
    ...
    for (const auto& tr : asr.m_results)
    {
      ...
      f << std::setprecision(std::numeric_limits<double>::max_digits10)
        << tr.m_time_ms_align << tr.m_time_ms_dmap << std::endl;
    }
  ...
  const std::string filepath =
      output_dir + "/" + "icp_p2p_" + m_level_icp_p2p + "_icp_p2pl_" +
      m_level_icp_p2pl + "_dmap_p2p_" + m_level_dmap_p2p + "_dmap_p2pl_" +
      m_level_dmap_p2pl + "_" + kLogFilenameAddInfo + ".txt";
  ...
  f << std::endl
    << std::endl
    << "=== 2D Laser Scan Aligners Benchmark --- RESULTS ===" << std::endl
    << "  scenes      : circle, square, corridor, random" << std::endl
    << "  trials/scene: " << cfg.m_num_trials << std::endl
    << "  trials top95: "
    << cfg.m_num_trials - static_cast<int>(std::floor(0.05f * cfg.m_num_trials))
    << std::endl
    << "  dmap res (m): " << kDistMapParamDistMapRes << std::endl
    << "  noise    (m): " << cfg.m_noise_std << std::endl
    << "  voxelization: " << (kEnableVoxelization ? "enabled" : "not enabled")
    << std::endl
    << std::endl
    << "  variable type icp_p2p  : "
    << (idx_variable_type_icp_p2p == -1
            ? "NOT FOUND"
            : my_types_str[idx_variable_type_icp_p2p])
    << std::endl
    << "  variable type icp_p2pl : "
    << (idx_variable_type_icp_p2pl == -1
            ? "NOT FOUND"
            : my_types_str[idx_variable_type_icp_p2pl])
    << std::endl
    << "  variable type dmap_p2p : "
    << (idx_variable_type_dmap_p2p == -1
            ? "NOT FOUND"
            : my_types_str[idx_variable_type_dmap_p2p])
    << std::endl
    << "  variable type dmap_p2pl: "
    << (idx_variable_type_dmap_p2pl == -1
            ? "NOT FOUND"
            : my_types_str[idx_variable_type_dmap_p2pl])
    << std::endl
    << std::endl;

  for (const auto& res : results)
  {
    f << "[" << res.m_scene_name << "] " << res.m_aligner_type << ":"
      << std::endl
      ...
      << "- time align mean     (ms): " << res.m_res.m_time_align_mean_ms
      << std::endl
      << "- time align std      (ms): " << res.m_res.m_time_align_std_ms
      << std::endl
      << "- time align min      (ms): " << res.m_res.m_time_align_min_ms
      << std::endl
      << "- time align max      (ms): " << res.m_res.m_time_align_max_ms
      << std::endl
      << "- time dmap mean     (ms): " << res.m_res.m_time_dmap_mean_ms
      << std::endl
      << "- time dmap std      (ms): " << res.m_res.m_time_dmap_std_ms
      << std::endl
      << "- time dmap min      (ms): " << res.m_res.m_time_dmap_min_ms
      << std::endl
      << "- time dmap max      (ms): " << res.m_res.m_time_dmap_max_ms
      << std::endl
      << std::endl
      << std::endl;
  }
         ...
         std::mt19937& rng_noise, const std::string& output_dir)
{
  std::vector<Scene> scenes = {
      {"circle",
       [&](PointCloudType& fixed, PointCloudType& moving,
           const EstimateType& gt)
       {
         msg::LaserScan scan_fixed, scan_moving;
         test::getLaser2DLaserScanMsgCircle(
             scan_fixed, EstimateType::Identity(), cfg.m_circ_radius,
             cfg.m_ang_res, cfg.m_fov, cfg.m_max_range, rng_noise,
             cfg.m_noise_std);
         test::getLaser2DLaserScanMsgCircle(
             scan_moving, gt, cfg.m_circ_radius, cfg.m_ang_res, cfg.m_fov,
             cfg.m_max_range, rng_noise, cfg.m_noise_std);
         laserScanToPointCloud(fixed, scan_fixed);
         laserScanToPointCloud(moving, scan_moving);
      ...
      {"square",
       ...
       {
         msg::LaserScan scan_fixed, scan_moving;
         test::getLaserScanMsgSquare(
             scan_fixed, EstimateType::Identity(), cfg.m_ang_res, cfg.m_fov,
             cfg.m_max_range, cfg.m_sqr_side, rng_noise, cfg.m_noise_std);
         test::getLaserScanMsgSquare(scan_moving, gt, cfg.m_ang_res, cfg.m_fov,
                                     cfg.m_max_range, cfg.m_sqr_side, rng_noise,
                                     cfg.m_noise_std);
         laserScanToPointCloud(fixed, scan_fixed);
         laserScanToPointCloud(moving, scan_moving);
         normal_computator.computeNormals(fixed);
      ...
      {"corridor",
       ...
       {
         msg::LaserScan scan_fixed, scan_moving;
         test::getLaserScanMsgCorridor(
             scan_fixed, EstimateType::Identity(), cfg.m_ang_res, cfg.m_fov,
             cfg.m_max_range, cfg.m_corr_width, cfg.m_corr_length, rng_noise,
             false, cfg.m_noise_std);
         test::getLaserScanMsgCorridor(scan_moving, gt, cfg.m_ang_res,
                                       cfg.m_fov, cfg.m_max_range,
                                       cfg.m_corr_width, cfg.m_corr_length,
      ...
      {"random", [&](PointCloudType& fixed, PointCloudType& moving,
                     const EstimateType& gt)
       {
         ...
         moving.transformInPlace(gt.inverse());
       }}};

  std::vector<AlignerSceneResults> results;

  std::cout << "=== 2D Laser Scan Aligners Benchmark ===" << std::endl
            << "  scenes      : circle, square, corridor, random" << std::endl
            << "  trials/scene: " << cfg.m_num_trials << std::endl;
  ...
  for (auto& scene : scenes)
  {
    ...
    for (int idx = 0; idx < cfg.m_num_trials; idx++)
    {
      ...
      scene.m_generate(*pc_fixed, *pc_moving, gt_t);
      ...
      if (is_rnd_scene)
      {
        ...
        std::function<void(void)> print_feedback = [&]()
        {
          std::cout << "  [trial " << (idx + 1) << "/" << cfg.m_num_trials
                    ...
                    << ") [time: align=" << icp_p2p_res.m_time_ms_align
                    << " ms ; dmap=" << icp_p2p_res.m_time_ms_dmap << " ms ]"
                    << std::endl
                    ...
                    << ") [time: align=" << dmap_p2p_res.m_time_ms_align
                    << " ms ; dmap=" << dmap_p2p_res.m_time_ms_dmap << " ms ]"
                    << std::endl;
        };
      ...
      else
      {
        ...
        std::function<void(void)> print_feedback = [&]()
        {
          std::cout << "  [trial " << (idx + 1) << "/" << cfg.m_num_trials
                    ...
                    << icp_p2pl_res.m_error_y << ")    err_r="
                    << geometry::radToDeg(icp_p2pl_res.m_error_rot) << " deg"
                    << " (estimate: " << icp_p2pl_res.m_estimate_x << " "
                    << icp_p2pl_res.m_estimate_y << " "
                    << geometry::radToDeg(icp_p2pl_res.m_estimate_th)
                    << ") [time: align=" << icp_p2pl_res.m_time_ms_align
                    << " ms ; dmap=" << icp_p2pl_res.m_time_ms_dmap << " ms ]"
                    << std::endl
                    << "    - [dmap pt2pt] err_t=" << dmap_p2p_res.m_error_trans
                    << " m (trans: " << dmap_p2p_res.m_error_x << " , "
                    << dmap_p2p_res.m_error_y << ")    err_r="
                    ...
                    << std::endl
                    << "    - [dmap pt2pl] err_t="
                    << dmap_p2pl_res.m_error_trans
                    << " m (trans: " << dmap_p2pl_res.m_error_x << " , "
                    << dmap_p2pl_res.m_error_y << ")    err_r="
                    << geometry::radToDeg(dmap_p2pl_res.m_error_rot) << " deg"
                    << " (estimate: " << dmap_p2pl_res.m_estimate_x << " "
                    << dmap_p2pl_res.m_estimate_y << " "
                    << geometry::radToDeg(dmap_p2pl_res.m_estimate_th)
                    << ") [time: align=" << dmap_p2pl_res.m_time_ms_align
                    << " ms ; dmap=" << dmap_p2pl_res.m_time_ms_dmap << " ms ]"
                    << std::endl;
        };
      ...
      if (idx == 0 && kEnableSaveDistanceMapExamples)
      {
        dmap_p2pl.setTarget(pc_fixed, EstimateType::Identity());

        data_structures::RGBImage idx_img;
        data_structures::RGBImage dist_img;
        data_structures::RGBImage dist_deriv_x_img;
        data_structures::RGBImage dist_deriv_y_img;
        data_structures::RGBImage dist_deriv_2_xx_img;
        data_structures::RGBImage dist_deriv_2_xy_img;
        data_structures::RGBImage dist_deriv_2_yx_img;
        data_structures::RGBImage dist_deriv_2_yy_img;

        auto* dmap = dmap_p2pl.getVariable()->measurement()->getDistanceMap();

        dmap->toImageIdx(idx_img);
        dmap->toImageDist(dist_img, *pc_fixed);
        dmap->toImageDistDx(dist_deriv_x_img, *pc_fixed);
        dmap->toImageDistDy(dist_deriv_y_img, *pc_fixed);
        dmap->toImageDistD2Dxx(dist_deriv_2_xx_img, *pc_fixed);
        dmap->toImageDistD2Dxy(dist_deriv_2_xy_img, *pc_fixed);
        dmap->toImageDistD2Dyx(dist_deriv_2_yx_img, *pc_fixed);
        dmap->toImageDistD2Dyy(dist_deriv_2_yy_img, *pc_fixed);

        dmap->drawPoints(idx_img, *pc_fixed, cv::Scalar(255, 0, 0));
        dmap->drawPoints(dist_img, *pc_fixed, cv::Scalar(255, 0, 0));
        dmap->drawPoints(dist_deriv_x_img, *pc_fixed, cv::Scalar(255, 0, 0));
        dmap->drawPoints(dist_deriv_y_img, *pc_fixed, cv::Scalar(255, 0, 0));
        dmap->drawPoints(dist_deriv_2_xx_img, *pc_fixed, cv::Scalar(255, 0, 0));
        dmap->drawPoints(dist_deriv_2_xy_img, *pc_fixed, cv::Scalar(255, 0, 0));
        dmap->drawPoints(dist_deriv_2_yx_img, *pc_fixed, cv::Scalar(255, 0, 0));
        dmap->drawPoints(dist_deriv_2_yy_img, *pc_fixed, cv::Scalar(255, 0, 0));

        cv::namedWindow("synthetic bechmark - scan: index");
        cv::namedWindow("synthetic bechmark - scan: distance");
        cv::namedWindow("synthetic bechmark - scan: deriv x");
        cv::namedWindow("synthetic bechmark - scan: deriv y");
        cv::namedWindow("synthetic bechmark - scan: 2nd deriv xx");
        cv::namedWindow("synthetic bechmark - scan: 2nd deriv xy");
        cv::namedWindow("synthetic bechmark - scan: 2nd deriv yx");
        cv::namedWindow("synthetic bechmark - scan: 2nd deriv yy");

        cv::imshow("synthetic bechmark - scan: index", idx_img);
        cv::imshow("synthetic bechmark - scan: distance", dist_img);
        cv::imshow("synthetic bechmark - scan: deriv x", dist_deriv_x_img);
        cv::imshow("synthetic bechmark - scan: deriv y", dist_deriv_y_img);
        cv::imshow("synthetic bechmark - scan: 2nd deriv xx",
                   dist_deriv_2_xx_img);
        cv::imshow("synthetic bechmark - scan: 2nd deriv xy",
                   dist_deriv_2_xy_img);
        cv::imshow("synthetic bechmark - scan: 2nd deriv yx",
                   dist_deriv_2_yx_img);
        cv::imshow("synthetic bechmark - scan: 2nd deriv yy",
                   dist_deriv_2_yy_img);

        cv::waitKey(0);

        cv::imwrite(output_dir + "/examples/" + scene.m_name + "_idx_img.png",
                    idx_img);
        cv::imwrite(output_dir + "/examples/" + scene.m_name + "_dist_img.png",
                    dist_img);
        cv::imwrite(
            output_dir + "/examples/" + scene.m_name + "_dist_deriv_x_img.png",
            dist_deriv_x_img);
        cv::imwrite(
            output_dir + "/examples/" + scene.m_name + "_dist_deriv_y_img.png",
            dist_deriv_y_img);
        cv::imwrite(output_dir + "/examples/" + scene.m_name +
                        "_dist_deriv_2_xx_img.png",
                    dist_deriv_2_xx_img);
        cv::imwrite(output_dir + "/examples/" + scene.m_name +
                        "_dist_deriv_2_xy_img.png",
                    dist_deriv_2_xy_img);
        cv::imwrite(output_dir + "/examples/" + scene.m_name +
                        "_dist_deriv_2_yx_img.png",
                    dist_deriv_2_yx_img);
        cv::imwrite(output_dir + "/examples/" + scene.m_name +
                        "_dist_deriv_2_yy_img.png",
                    dist_deriv_2_yy_img);
  ...
  std::cout << std::endl
            << std::endl
            << "=== 2D Laser Scan Aligners Benchmark --- RESULTS ==="
            << std::endl
            << "  scenes      : circle, square, corridor, random" << std::endl
            << "  trials/scene: " << cfg.m_num_trials << std::endl
            << "  trials top95: "
            ...
            << "  noise    (m): " << cfg.m_noise_std << std::endl
            << "  voxelization: "
            << (kEnableVoxelization ? "enabled" : "not enabled") << std::endl
            << std::endl;

  for (const auto& res : results)
  {
    std::cout << "[" << res.m_scene_name << "] " << res.m_aligner_type << ":"
              << std::endl
              << "- err trans rmse (m): " << res.m_res.m_error_trans
              << std::endl
              << "- err x rmse     (m): " << res.m_res.m_error_x << std::endl
              << "- err y rmse     (m): " << res.m_res.m_error_y << std::endl
              << "- err rot rmse (deg): "
              << geometry::radToDeg(res.m_res.m_error_rot) << std::endl
              << "- mean chi sqr      : " << res.m_res.m_chi_sqr << std::endl
              << "- mean chi sqr 95%  : " << res.m_res.m_chi_sqr_mean95
              << std::endl
              << "- time align mean     (ms): "
              << res.m_res.m_time_align_mean_ms << std::endl
              << "- time align std      (ms): " << res.m_res.m_time_align_std_ms
              << std::endl
              << "- time align min      (ms): " << res.m_res.m_time_align_min_ms
              << std::endl
  ...
  saveTrialResultsCSV(results, output_dir, cfg, icp_p2p, icp_p2pl, dmap_p2p,
                      dmap_p2pl);
}
