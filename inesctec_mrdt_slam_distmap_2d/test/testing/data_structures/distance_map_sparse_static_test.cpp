...
TEST(data_structures_distance_map_sparse_static_generic, constructor)
{
  DistanceMapSparseStaticFloat dist_map_std_f;
  DistanceMapSparseStaticDouble dist_map_std_d;

  DistanceMapSparseStaticWith2ndDerivFloat dist_map_with_d2_f;
  DistanceMapSparseStaticWith2ndDerivDouble dist_map_with_d2_d;
...
TEST(data_structures_distance_map_sparse_static_generic, findNeighbors)
{
  constexpr int rows = 50;
  constexpr int cols = 100;

  DistanceMapSparseStaticFloat map_std;
  DistanceMapSparseStaticWith2ndDerivFloat map_with_d2;

  ASSERT_NO_FATAL_FAILURE(map_std.getMap()->resize(rows, cols));
  ASSERT_NO_FATAL_FAILURE(map_with_d2.getMap()->resize(rows, cols));

  DistanceMapSparseStaticFloat::CellIndex nn_std[8];
  DistanceMapSparseStaticWith2ndDerivFloat::CellIndex nn_with_d2[8];
...
TEST(data_structures_distance_map_sparse_static_standard, setGoalsSimpleParam)
{
  using DistanceMapType = DistanceMapSparseStaticFloat;
  ...
  EXPECT_FLOAT_EQ(map.getMaxDist(), -1.f);
}  // TEST(data_structures_distance_map_sparse_static_standard,
   // setGoalsSimpleParam)

TEST(data_structures_distance_map_sparse_static_with_2nd_deriv,
     setGoalsSimpleParam)
{
  using DistanceMapType = DistanceMapSparseStaticWith2ndDerivFloat;
  using PointCloudType = pcl::Point2VectorCloud_<DistanceMapType::Scalar>;
  using PointCloudTypePtr = std::shared_ptr<PointCloudType>;
  using PointType = PointCloudType::PointType;
  using PointVectorType = DistanceMapType::PointVectorType;
...
TEST(data_structures_distance_map_sparse_static_standard, setGoalsSimpleSize)
{
  using DistanceMapType = DistanceMapSparseStaticFloat;
  using PointCloudType = pcl::Point2VectorCloud_<DistanceMapType::Scalar>;
  using PointCloudTypePtr = std::shared_ptr<PointCloudType>;
...
TEST(data_structures_distance_map_sparse_static_standard, setGoalsSimple)
{
  using DistanceMapType = DistanceMapSparseStaticFloat;
  using PointCloudType = pcl::Point2VectorCloud_<DistanceMapType::Scalar>;
  using PointCloudTypePtr = std::shared_ptr<PointCloudType>;
  ...
  EXPECT_NEAR((*map.getMap()).coeff(100, 100).m_dist, 0.f, res * 1e-2);
  EXPECT_NEAR((*map.getMap()).coeff(100, 99).m_dist, .01f, res * 1e-2);
  EXPECT_NEAR((*map.getMap()).coeff(100, 101).m_dist, .01f, res * 1e-2);
  EXPECT_NEAR((*map.getMap()).coeff(99, 100).m_dist, .01f, res * 1e-2);
  EXPECT_NEAR((*map.getMap()).coeff(101, 100).m_dist, .01f, res * 1e-2);

  EXPECT_NEAR((*map.getMap()).coeff(100, 98).m_dist, .02f, res * 1e-2);
  EXPECT_NEAR((*map.getMap()).coeff(100, 102).m_dist, .02f, res * 1e-2);
  EXPECT_NEAR((*map.getMap()).coeff(98, 100).m_dist, .02f, res * 1e-2);
  EXPECT_NEAR((*map.getMap()).coeff(102, 100).m_dist, .02f, res * 1e-2);

  EXPECT_NEAR((*map.getMap()).coeff(100, 98).m_dist_d(0),
              ((*map.getMap()).coeff(101, 98).m_dist -
               (*map.getMap()).coeff(99, 98).m_dist) *
                  .5f / res,
              res * 1e-2);
  EXPECT_NEAR((*map.getMap()).coeff(100, 102).m_dist_d(0),
              ((*map.getMap()).coeff(101, 102).m_dist -
               (*map.getMap()).coeff(99, 102).m_dist) *
                  .5f / res,
              res * 1e-2);
  EXPECT_NEAR((*map.getMap()).coeff(98, 100).m_dist_d(0),
              ((*map.getMap()).coeff(99, 100).m_dist -
               (*map.getMap()).coeff(97, 100).m_dist) *
                  .5f / res,
              res * 1e-2);
  EXPECT_NEAR((*map.getMap()).coeff(102, 100).m_dist_d(0),
              ((*map.getMap()).coeff(103, 100).m_dist -
               (*map.getMap()).coeff(101, 100).m_dist) *
                  .5f / res,
              res * 1e-2);

  EXPECT_NEAR((*map.getMap()).coeff(100, 98).m_dist_d(1),
              ((*map.getMap()).coeff(100, 99).m_dist -
               (*map.getMap()).coeff(100, 97).m_dist) *
                  .5f / res,
              res * 1e-2);
  EXPECT_NEAR((*map.getMap()).coeff(100, 102).m_dist_d(1),
              ((*map.getMap()).coeff(100, 103).m_dist -
               (*map.getMap()).coeff(100, 101).m_dist) *
                  .5f / res,
              res * 1e-2);
  EXPECT_NEAR((*map.getMap()).coeff(98, 100).m_dist_d(1),
              ((*map.getMap()).coeff(98, 101).m_dist -
               (*map.getMap()).coeff(98, 99).m_dist) *
                  .5f / res,
              res * 1e-2);
  EXPECT_NEAR((*map.getMap()).coeff(102, 100).m_dist_d(1),
              ((*map.getMap()).coeff(102, 101).m_dist -
               (*map.getMap()).coeff(102, 99).m_dist) *
                  .5f / res,
              res * 1e-2);

  EXPECT_NEAR((*map.getMap()).coeff(0, 0).m_dist, 1.f, res * 1e-2);

  for (int r = 0; r < map.getMap()->rows(); r++)
  {
    for (int c = 0; c < map.getMap()->cols(); c++)
    {
      if ((*map.getMap()).coeff(r, c).m_nn_idx >= 0)
      {
        EXPECT_EQ((*map.getMap()).coeff(r, c).m_nn_idx, 0);
      }
    }
  }
  ...
  EXPECT_NEAR(map_cell.m_dist,
              0.25f * ((*map.getMap()).coeff(100, 100).m_dist +
                       (*map.getMap()).coeff(101, 100).m_dist +
                       (*map.getMap()).coeff(100, 101).m_dist +
                       (*map.getMap()).coeff(101, 101).m_dist),
              res * 1e-2);
  EXPECT_NEAR(map_cell.m_dist_d(0),
              0.25f * ((*map.getMap()).coeff(100, 100).m_dist_d(0) +
                       (*map.getMap()).coeff(101, 100).m_dist_d(0) +
                       (*map.getMap()).coeff(100, 101).m_dist_d(0) +
                       (*map.getMap()).coeff(101, 101).m_dist_d(0)),
              res * 1e-2);
  EXPECT_NEAR(map_cell.m_dist_d(1),
              0.25f * ((*map.getMap()).coeff(100, 100).m_dist_d(1) +
                       (*map.getMap()).coeff(101, 100).m_dist_d(1) +
                       (*map.getMap()).coeff(100, 101).m_dist_d(1) +
                       (*map.getMap()).coeff(101, 101).m_dist_d(1)),
              res * 1e-2);
  ...
  map.toImageCellsSet(cells_set_img);
  ...
  map.drawPoints(cells_set_img, *pc, cv::Scalar(255, 0, 0));
  ...
  cv::namedWindow("std::setGoalsSimple: cells set");
  ...
  cv::imshow("std::setGoalsSimple: cells set", cells_set_img);
...
TEST(data_structures_distance_map_sparse_static_with_2nd_deriv, setGoalsSimple)
{
  using DistanceMapType = DistanceMapSparseStaticWith2ndDerivFloat;
  using PointCloudType = pcl::Point2VectorCloud_<DistanceMapType::Scalar>;
  using PointCloudTypePtr = std::shared_ptr<PointCloudType>;
  ...
  cv::namedWindow("2nd::setGoalsSimple: cells set");
  ...
  cv::imshow("2nd::setGoalsSimple: cells set", cells_set_img);
...
TEST(data_structures_distance_map_sparse_static_standard, getValueWith2ndDeriv)
{
  using Scalar = float;

  using DistanceMapGTType =
      DistanceMapSparseStatic_<DistanceMapWith2ndDerivCell_<Scalar>>;
  ...
  using DistanceMapType = DistanceMapSparseStatic_<DistanceMapCell_<Scalar>>;
  using PointCloudType = pcl::Point2VectorCloud_<DistanceMapType::Scalar>;
  using PointCloudTypePtr = std::shared_ptr<PointCloudType>;
  using PointType = PointCloudType::PointType;
  using PointVectorType = DistanceMapType::PointVectorType;
       ...
       pt_x <= map.getMap()->rows() * map.getRes(); pt_x += map.getRes())
  {
         ...
         pt_y <= map.getMap()->cols() * map.getRes(); pt_y += map.getRes())
    {
      ...
      EXPECT_NEAR(cell.m_dist, cell_gt.m_dist, res * 1e-2);
      EXPECT_NEAR(cell.m_dist_d(0), cell_gt.m_dist_d(0), res * 1e-2);
      EXPECT_NEAR(cell.m_dist_d(1), cell_gt.m_dist_d(1), res * 1e-2);
      EXPECT_NEAR(cell.m_dist_d2(0, 0), cell_gt.m_dist_d2(0, 0), res * 1e-2);
      EXPECT_NEAR(cell.m_dist_d2(0, 1), cell_gt.m_dist_d2(0, 1), res * 1e-2);
      EXPECT_NEAR(cell.m_dist_d2(1, 0), cell_gt.m_dist_d2(1, 0), res * 1e-2);
      EXPECT_NEAR(cell.m_dist_d2(1, 1), cell_gt.m_dist_d2(1, 1), res * 1e-2);
    }
  }
  ...
  cv::namedWindow("std::getValueWith2ndDeriv: cells set");
  ...
  cv::imshow("std::getValueWith2ndDeriv: cells set", cells_set_img);
     ...
     getValueWith2ndDeriv)
{
  ...
  cv::namedWindow("2nd::getValueWith2ndDeriv: cells set");
  ...
  cv::imshow("2nd::getValueWith2ndDeriv: cells set", cells_set_img);
     ...
     getValueSubPxWith2ndDeriv)
{
       ...
       pt_x += map.getRes())
  {
         ...
         pt_y += map.getRes())
    {
      ...
      EXPECT_NEAR(cell.m_dist_d(0), cell_gt.m_dist_d(0), res * 1e-2);
      EXPECT_NEAR(cell.m_dist_d(1), cell_gt.m_dist_d(1), res * 1e-2);
      EXPECT_NEAR(cell.m_dist_d2(0, 0), cell_gt.m_dist_d2(0, 0), res * 0.5);
      EXPECT_NEAR(cell.m_dist_d2(0, 1), cell_gt.m_dist_d2(0, 1), res * 0.5);
      EXPECT_NEAR(cell.m_dist_d2(1, 0), cell_gt.m_dist_d2(1, 0), res * 0.5);
      EXPECT_NEAR(cell.m_dist_d2(1, 1), cell_gt.m_dist_d2(1, 1), res * 0.5);
    }
  }
}  // TEST(data_structures_distance_map_sparse_static_standard,
   // getValueSubPxWith2ndDeriv)
...
TEST(data_structures_distance_map_sparse_static_standard, setGoalsRandom)
{
  using Scalar = float;
  ...
  EXPECT_NO_THROW(map.toImageCellsSet(cells_set_img));
  ...
  EXPECT_NO_THROW(map.drawPoints(cells_set_img, *pc, cv::Scalar(255, 0, 0)));
  ...
  cv::namedWindow("std::setGoalsRandom: cells set");
  ...
  cv::imshow("std::setGoalsRandom: cells set", cells_set_img);
...
TEST(data_structures_distance_map_sparse_static_with_2nd_deriv, setGoalsRandom)
{
  using Scalar = float;
  ...
  cv::namedWindow("2nd::setGoalsRandom: cells set");
  ...
  cv::imshow("2nd::setGoalsRandom: cells set", cells_set_img);
...
TEST(data_structures_distance_map_sparse_static_standard, validation)
{
  using Scalar = float;
  ...
  for (int r = 0; r < (*map.getMap()).rows(); r++)
  {
    for (int c = 0; c < (*map.getMap()).cols(); c++)
    {
      ...
      EXPECT_NEAR((*map.getMap()).coeff(r, c).m_dist,
                  (*map_gt.getMap()).coeff(r, c).m_dist, res * 1e-2);
      EXPECT_NEAR((*map.getMap()).coeff(r, c).m_dist_d(0),
                  (*map_gt.getMap()).coeff(r, c).m_dist_d(0), res * 1e-2);
      EXPECT_NEAR((*map.getMap()).coeff(r, c).m_dist_d(1),
                  (*map_gt.getMap()).coeff(r, c).m_dist_d(1), res * 1e-2);
      EXPECT_NEAR(cell.m_dist_d2(0, 0),
                  (*map_gt.getMap()).coeff(r, c).m_dist_d2(0, 0), res * 1e-2);
      EXPECT_NEAR(cell.m_dist_d2(0, 1),
                  (*map_gt.getMap()).coeff(r, c).m_dist_d2(0, 1), res * 1e-2);
      EXPECT_NEAR(cell.m_dist_d2(1, 0),
                  (*map_gt.getMap()).coeff(r, c).m_dist_d2(1, 0), res * 1e-2);
      EXPECT_NEAR(cell.m_dist_d2(1, 1),
                  (*map_gt.getMap()).coeff(r, c).m_dist_d2(1, 1), res * 1e-2);

      if ((*map.getMap()).coeff(r, c).m_dist !=
          (*map_gt.getMap()).coeff(r, c).m_dist)
        pc_diff_map_dist->emplace_back(PointType(PointVectorType(r, c)));
      if ((*map.getMap()).coeff(r, c).m_dist_d(0) !=
          (*map_gt.getMap()).coeff(r, c).m_dist_d(0))
        pc_diff_map_dist_dx->emplace_back(PointType(PointVectorType(r, c)));
      if ((*map.getMap()).coeff(r, c).m_dist_d(1) !=
          (*map_gt.getMap()).coeff(r, c).m_dist_d(1))
        pc_diff_map_dist_dy->emplace_back(PointType(PointVectorType(r, c)));
      if (cell.m_dist_d2(0, 0) !=
          (*map_gt.getMap()).coeff(r, c).m_dist_d2(0, 0))
        pc_diff_map_dist_d2dxx->emplace_back(PointType(PointVectorType(r, c)));
      if (cell.m_dist_d2(0, 1) !=
          (*map_gt.getMap()).coeff(r, c).m_dist_d2(0, 1))
        pc_diff_map_dist_d2dxy->emplace_back(PointType(PointVectorType(r, c)));
      if (cell.m_dist_d2(1, 0) !=
          (*map_gt.getMap()).coeff(r, c).m_dist_d2(1, 0))
        pc_diff_map_dist_d2dyx->emplace_back(PointType(PointVectorType(r, c)));
      if (cell.m_dist_d2(1, 1) !=
          (*map_gt.getMap()).coeff(r, c).m_dist_d2(1, 1))
        pc_diff_map_dist_d2dyy->emplace_back(PointType(PointVectorType(r, c)));

      EXPECT_NEAR(cell.m_dist, (*map_gt.getMap()).coeff(r, c).m_dist,
                  res * 1e-2);
      EXPECT_NEAR(cell.m_dist_d(0), (*map_gt.getMap()).coeff(r, c).m_dist_d(0),
                  res * 1e-2);
      EXPECT_NEAR(cell.m_dist_d(1), (*map_gt.getMap()).coeff(r, c).m_dist_d(1),
                  res * 1e-2);
      EXPECT_NEAR(cell.m_dist_d2(0, 0),
                  (*map_gt.getMap()).coeff(r, c).m_dist_d2(0, 0), res * 1e-2);
      EXPECT_NEAR(cell.m_dist_d2(0, 1),
      ...
      if (cell.m_dist != (*map_gt.getMap()).coeff(r, c).m_dist)
        pc_diff_get_dist->emplace_back(PointType(PointVectorType(r, c)));
      if (cell.m_dist_d(0) != (*map_gt.getMap()).coeff(r, c).m_dist_d(0))
        pc_diff_get_dist_dx->emplace_back(PointType(PointVectorType(r, c)));
      if (cell.m_dist_d(1) != (*map_gt.getMap()).coeff(r, c).m_dist_d(1))
...
TEST(data_structures_distance_map_sparse_static_with_2nd_deriv, validation)
{
  using Scalar = float;
  ...
  for (int r = 0; r < (*map.getMap()).rows(); r++)
  {
    for (int c = 0; c < (*map.getMap()).cols(); c++)
    {
      ...
      EXPECT_NEAR((*map.getMap()).coeff(r, c).m_dist_d(1),
                  (*map_gt.getMap()).coeff(r, c).m_dist_d(1), res * 1e-2);
      EXPECT_NEAR((*map.getMap()).coeff(r, c).m_dist_d2(0, 0),
                  (*map_gt.getMap()).coeff(r, c).m_dist_d2(0, 0), res * 1e-2);
      EXPECT_NEAR((*map.getMap()).coeff(r, c).m_dist_d2(0, 1),
                  (*map_gt.getMap()).coeff(r, c).m_dist_d2(0, 1), res * 1e-2);
      EXPECT_NEAR((*map.getMap()).coeff(r, c).m_dist_d2(1, 0),
                  (*map_gt.getMap()).coeff(r, c).m_dist_d2(1, 0), res * 1e-2);
      EXPECT_NEAR((*map.getMap()).coeff(r, c).m_dist_d2(1, 1),
                  (*map_gt.getMap()).coeff(r, c).m_dist_d2(1, 1), res * 1e-2);
      ...
      if ((*map.getMap()).coeff(r, c).m_dist_d(1) !=
          (*map_gt.getMap()).coeff(r, c).m_dist_d(1))
        pc_diff_map_dist_dy->emplace_back(PointType(PointVectorType(r, c)));
      if ((*map.getMap()).coeff(r, c).m_dist_d2(0, 0) !=
          (*map_gt.getMap()).coeff(r, c).m_dist_d2(0, 0))
        pc_diff_map_dist_d2dxx->emplace_back(PointType(PointVectorType(r, c)));
      if ((*map.getMap()).coeff(r, c).m_dist_d2(0, 1) !=
          (*map_gt.getMap()).coeff(r, c).m_dist_d2(0, 1))
        pc_diff_map_dist_d2dxy->emplace_back(PointType(PointVectorType(r, c)));
      if ((*map.getMap()).coeff(r, c).m_dist_d2(1, 0) !=
          (*map_gt.getMap()).coeff(r, c).m_dist_d2(1, 0))
        pc_diff_map_dist_d2dyx->emplace_back(PointType(PointVectorType(r, c)));
      if ((*map.getMap()).coeff(r, c).m_dist_d2(1, 1) !=
          (*map_gt.getMap()).coeff(r, c).m_dist_d2(1, 1))
        pc_diff_map_dist_d2dyy->emplace_back(PointType(PointVectorType(r, c)));
...
TEST(data_structures_distance_map_sparse_static_standard, serializerBoss)
{
  using DistanceMapType = DistanceMapSparseStaticFloat;
  ...
  /**
   * @todo the serialization of directly the distance map container does not
   *       work with the point cloud...
   * @note also, do not forget to do $ export LC_ALL=C (SRRG BOSS stuff....)
   */
}  // TEST(data_structures_distance_map_sparse_static_standard, serializerBoss)

TEST(data_structures_distance_map_sparse_static_with_2nd_deriv, serializerBoss)
{
  using DistanceMapType = DistanceMapSparseStaticWith2ndDerivFloat;
  using DistanceMapTypePtr = std::shared_ptr<DistanceMapType>;
  using PointCloudType = pcl::Point2VectorCloud_<DistanceMapType::Scalar>;
...
TEST(data_structures_distance_map_sparse_static_generic, sizeDenseVsSparses)
{
  using Scalar = float;
  ...
  using DistanceMapCellType = DistanceMapCell_<Scalar>;
  using DistanceMapDenseType = DistanceMapStatic_<DistanceMapCellType>;
  using DistanceMapSparseType = DistanceMapSparseStatic_<DistanceMapCellType>;

  std::random_device rd;
  std::mt19937 rng(rd());

  DistanceMapDenseType map_dense;
  DistanceMapSparseType map_sparse;
  ...
  Scalar map_size = Scalar(10.0);
  Scalar max_range = Scalar(10.0);
  Scalar max_dist = Scalar(0.30);

  Scalar ang_res = geometry::degToRad(static_cast<Scalar>(0.125));
  Scalar fov = geometry::degToRad(static_cast<Scalar>(360.0));

  test::getLaser2DPointCloudRandom(*pc, ang_res, fov, max_range, rng);

  ASSERT_EQ(pc->size(), 2880);

  std::cout << FG_SC_BMAGENTA("time maps setGoals stats:") << std::endl;

  PRINT_AND_LOG_TIME("map_dense.setGoals", EXPECT_NO_THROW(map_dense.setGoals(
                                               *pc, res, map_size, max_dist)));
  PRINT_AND_LOG_TIME("map_sparse.setGoals", EXPECT_NO_THROW(map_sparse.setGoals(
                                                *pc, res, map_size, max_dist)));

  ASSERT_EQ(map_dense.getMap()->rows(), map_sparse.getMap()->rows());
  ASSERT_EQ(map_dense.getMap()->cols(), map_sparse.getMap()->cols());

  std::cout << FG_SC_BBLUE("distance map parameters:") << std::endl
            << "- res      : " << res << " (m)" << std::endl
            << "- map_size : " << map_size << " (m)" << std::endl
            << "- max_dist : " << max_dist << " (m)" << std::endl
            << FG_SC_BYELLOW("generic stats:") << std::endl
            << "- rows: " << map_dense.getMap()->rows() << std::endl
            << "- cols: " << map_dense.getMap()->cols() << std::endl
            << "- %occ: "
            << static_cast<double>(map_sparse.getMap()->size()) * 100.0 /
                   (map_dense.getMap()->rows() * map_dense.getMap()->cols())
            << std::endl
            << FG_SC_BGREEN("dense size : " << map_dense.size()) << std::endl
            << "- size()  : " << map_dense.getMap()->size() << std::endl
            << FG_SC_BGREEN("sparse size: " << map_sparse.size()) << std::endl
            << "- #buckets: " << map_sparse.getMap()->bucket_count()
            << std::endl
            << "- size()  : " << map_sparse.getMap()->size() << std::endl
            << FG_SC_BRED("%memory sparse / dense: "
                          << static_cast<double>(map_sparse.size()) * 100.0 /
                                 map_dense.size())
            << std::endl
            << std::endl;

  ang_res = geometry::degToRad(static_cast<Scalar>(5.0));

  test::getLaser2DPointCloudRandom(*pc, ang_res, fov, max_range, rng);

  ASSERT_EQ(pc->size(), 72);
  ...
  ang_res = geometry::degToRad(static_cast<Scalar>(0.1));

  test::getLaser2DPointCloudRandom(*pc, ang_res, fov, max_range, rng);

  ASSERT_EQ(pc->size(), 3600);

  res = Scalar(0.01);
  map_size = Scalar(10.0);
  max_range = Scalar(10.0);
  max_dist = Scalar(-1.0);
  ...
  std::cout << FG_SC_BBLUE("distance map parameters:") << std::endl
            ...
            << std::endl
            << std::endl;
}  // TEST(data_structures_distance_map_sparse_static_generic,
   // sizeDenseVsSparses)
