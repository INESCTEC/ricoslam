...
{
  using VwrPointCloudType = srrg2_core::Point3fVectorCloud;
...
int main(int argc, char** argv)
{
  ...
  srrg_canvas = srrg_viewer->getCanvas(
      "srrg2_solver_se2_distance_map_point_point_left_error_factor");
  srrg_canvas->setName(
      "srrg2_solver_se2_distance_map_point_point_left_error_factor");
  ...
  std::thread run_tests(
      [&srrg_viewer]()
      {
        ...
        constexpr float noise_std = res;

        std::random_device rd;
        std::mt19937 rng(rd());

        float ang_res = inesctec_mrdt_slam_distmap_2d::geometry::degToRad(1.f);
        float fov = 2 * M_PIf32;

        std::cout << "TEST "
                     "SE2DistanceMapStaticPointPointLeftErrorFactorCorresponden"
                     "ceFreeDriven"
                  << std::endl
