...
int main(int argc, char** argv)
{
  ...
  srrg_canvas = srrg_viewer->getCanvas(
      "srrg2_solver_se2_distance_map_point_plane_error_factor");
  srrg_canvas->setName(
      "srrg2_solver_se2_distance_map_point_plane_error_factor");
  ...
  std::thread run_tests(
      [&srrg_viewer]()
      {
        ...
        std::cout << "TEST "
                     "SE2DistanceMapStaticPointPlaneErrorFactorCorrespondenceFr"
                     "eeDriven"
                  << std::endl
        ...
        while (srrg_viewer->isRunning())
        {
          ...
          {
            std::cout << "TEST "
                         "SE2DistanceMapStaticPointPlaneErrorFactorCorresponden"
                         "ceFreeDriven"
                      << std::endl
                      << "esc : exit the test (NEED TO INPUT ANOTHER LETTER "
                         "AFTER THE "
                         "CANVAS CLOSES!)"
                      << std::endl
                      << "w, s: forward linear motion" << std::endl
                      << "a, d: side linear motion" << std::endl
                      << "q, e: rotate" << std::endl
                      << "+, -: add / reduce noise (current noise: "
                      << noise_std << ")" << std::endl
                      << "1   : compute solver" << std::endl
                      << "c   : circle" << std::endl
                      << "x   : square" << std::endl
