...
class AlignerDistanceMapPointPlane_
    : public Aligner_<VariableSE2DistanceMapRightType_>
{
 public:
  ...
  using ThisType =
      AlignerDistanceMapPointPlane_<VariableSE2DistanceMapRightType_>;
  using BaseType = Aligner_<VariableSE2DistanceMapRightType_>;

  /**
   * @brief this is just defined as this to avoid compilation errors
   */
  using FactorType = std::conditional_t<
      std::is_same<PointType, pcl::PointNormal2_<Scalar>>::value,
      srrg2_solver::
 ...
 protected:
  ...
  FactorTypePtr m_factor_ =
      std::make_shared<FactorType>();  //!< SE(2) point-to-plane distance
 ...
 public:
  ...
  {
    assert(source &&
           "AlignerDistanceMapPointPlane_::setSource | source point cloud not "
           "set");
  ...
  {
    assert(source &&
           "AlignerDistanceMapPointPlane_::setSource | source SE2 variable not "
           "set");
    assert(source->measurement() &&
           "AlignerDistanceMapPointPlane_::setSource | source measurement not "
           "set");
    assert(source->measurement()->getPointCloudPtr() &&
           "AlignerDistanceMapPointPlane_::setSource | source point cloud "
           "shared ptr not set");
  ...
  {
    assert(target &&
           "AlignerDistanceMapPointPlane_::setTarget | target point cloud not "
           "set");
  ...
  {
    assert(target &&
           "AlignerDistanceMapPointPlane_::setTarget | target SE2 variable not "
           "set");
    assert(target->measurement() &&
           "AlignerDistanceMapPointPlane_::setTarget | target measurement not "
           "set");
    assert(target->measurement()->getPointCloudPtr() &&
           "AlignerDistanceMapPointPlane_::setTarget | target point cloud "
           "shared ptr not set");
  ...
  virtual void updateDbgOPGLCanvas(bool update_canvas = true) final
  {
    ...
    if (this->m_param_.m_dbg_ogl_show_normals)
    {
      m_dbg_ogl_pts_factor_normals_.resize(idx);

      this->m_dbg_ogl_canvas_->pushColor();
      this->m_dbg_ogl_canvas_->setColor(BaseType::OGLPointCloudTargetColorPts);
      ...
      this->m_dbg_ogl_canvas_->putPoints(m_dbg_ogl_pts_factor_normals_);
 ...
 private:
  ...
  inline void checkPointType() const
  {
    throw std::runtime_error(
        "AlignerDistanceMapPointPlane_::checkPointType | "
        "point type is not pcl::PointNormal2_<Scalar>! "
        "Distance map-based point-to-plane aligner is only "
        "implemented for pcl::PointNormal2_<Scalar> point types. "
        "Check if the VariableType implements a standard point cloud or one "
        "with normals.");
  }
...
};  // class AlignerDistanceMapPointPlane_<VariableSE2DistanceMapRightType_> :
