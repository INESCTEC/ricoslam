...
class AlignerDistanceMapICPPointPlane_
    : public Aligner_<VariableSE2DistanceMapRightType_>
{
 public:
  ...
  using ThisType =
      AlignerDistanceMapICPPointPlane_<VariableSE2DistanceMapRightType_>;
 ...
 public:
  ...
  AlignerDistanceMapICPPointPlane_(
                         ...
                         const EstimateType& sensor_in_robot) final
  {
    assert(
        source &&
        "AlignerDistanceMapICPPointPlane_::setSource | source point cloud not "
        "set");
  ...
  virtual void setSource(VariableType* const source) final
  {
    assert(
        source &&
        "AlignerDistanceMapICPPointPlane_::setSource | source SE2 variable not "
        "set");
    assert(
        source->measurement() &&
        "AlignerDistanceMapICPPointPlane_::setSource | source measurement not "
        "set");
    assert(source->measurement()->getPointCloudPtr() &&
           "AlignerDistanceMapICPPointPlane_::setSource | source point cloud "
           "shared ptr not set");
                         ...
                         const EstimateType& sensor_in_robot) final
  {
    assert(
        target &&
        "AlignerDistanceMapICPPointPlane_::setTarget | target point cloud not "
        "set");
  ...
  virtual void setTarget(VariableType* const target) final
  {
    assert(
        target &&
        "AlignerDistanceMapICPPointPlane_::setTarget | target SE2 variable not "
        "set");
    assert(
        target->measurement() &&
        "AlignerDistanceMapICPPointPlane_::setTarget | target measurement not "
        "set");
    assert(target->measurement()->getPointCloudPtr() &&
           "AlignerDistanceMapICPPointPlane_::setTarget | target point cloud "
           "shared ptr not set");
 ...
 private:
  ...
  inline void checkPointType() const
  {
    throw std::runtime_error(
        "AlignerDistanceMapICPPointPlane_::checkPointType | "
        "point type is not pcl::PointNormal2_<Scalar>! "
        "Distance map-based point-to-plane ICP aligner is only "
...
};  // class AlignerDistanceMapICPPointPlane_<VariableSE2DistanceMapRightType_>
