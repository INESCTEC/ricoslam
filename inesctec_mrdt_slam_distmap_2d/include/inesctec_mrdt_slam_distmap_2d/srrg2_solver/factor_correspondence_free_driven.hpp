...
class FactorCorrespondenceFreeDriven_ : public FactorBaseType_
{
 public:

  using ThisType =
      FactorCorrespondenceFreeDriven_<FactorBaseType_, MovingContainerType_>;
 ...
 protected:
  ...
  const MovingContainerType* m_moving_ =
 ...
 public:
   ...
   * @note container must be handle outside of the factor!
  ...
  const MovingContainerType* getMoving() const { return m_moving_; }
  ...
  srrg2_solver::FactorBase*& get() override
  {
    assert(((!m_info_matrix_vector_) ||
           ...
           "FactorCorrespondenceFreeDriven_::get | "
  ...
  virtual bool getNext(srrg2_solver::FactorBase*& moving)
  {
    assert(((!m_info_matrix_vector_) ||
            (m_info_matrix_vector_->size() == m_moving_->size())) &&
           "FactorCorrespondenceFreeDriven_::getNext | "
...
};  // class FactorCorrespondenceFreeDriven_<FactorBaseType_,
