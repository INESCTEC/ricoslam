...
/**
 * @brief sparse distance map template class to compute the distance and
 *        respective derivatives to the closest nearborhood cells based on a set
 *        of goals (dense matrix map structure = Eigen matrix)
 * @note sparse implementation is based on the std::unordered_map wrapped by the
 *       matrix sparse container, this choice was due to being an STL container,
 *       while also ensuring that, upon conflict on the hash keys, no memory is
 *       lost, though sacrificing some computational performance
 *       (O(1) >>> O(N) within the bucket...)
 * @note another reason is that upon rehashing the std::unordered_map does not
 *       reallocate memory, allowing to still use the cell pointer memory
 *       address to define the parent's nearest neighbor of the cell...
 * @tparam CellType_ distance map cell type
 */
template <typename CellType_>
class DistanceMapSparseStatic_ : public srrg2_core::Serializable
{
 public:

  using ThisType = DistanceMapSparseStatic_<CellType_>;
  ...
  using MapType = MatrixSparse_<CellType, CellTraits, CellIndex, CellIndexHash>;
  using MapTypePtr = std::unique_ptr<MapType>;

  using CellSearchQueueType = DistanceMapSparseCellSearchQueue_<CellType>;
 ...
 public:
   ...
   */
  inline int findNeighbors(CellIndex neighbors[], CellIndexScalar r,
                           CellIndexScalar c)
  {
    CellIndexScalar rmin = static_cast<int>(r) - 1 < 0 ? 0 : r - 1;
    CellIndexScalar rmax =
        r + 1 > m_map_->rows() - 1 ? m_map_->rows() - 1 : r + 1;

    CellIndexScalar cmin = static_cast<int>(c) - 1 < 0 ? 0 : c - 1;
    CellIndexScalar cmax =
        c + 1 > m_map_->cols() - 1 ? m_map_->cols() - 1 : c + 1;

    int k = 0;

    for (CellIndexScalar rr = rmin; rr <= rmax; rr++)
    {
      for (CellIndexScalar cc = cmin; cc <= cmax; cc++)
      {
        if ((rr != r) || (cc != c))
        {
          neighbors[k] = CellIndex(rr, cc);
          k++;
        }
      }
    }
    ...
    return k;
  }  // inline int DistanceMapSparseStatic_::findNeighbors(CellIndex[],
     // CellIndexScalar, CellIndexScalar)
  ...
  {
    assert(res > 0 &&
           "DistanceMapSparseStatic_::setGoals | "
    ...
    for (size_t idx = 0; idx < pc.size(); idx++)
    {
      ...
      CellType* cell = &(*m_map_).coeffRef(map_idx.x(), map_idx.y());
      CellIndex cell_idx = CellIndex(static_cast<CellIndexScalar>(map_idx.x()),
                                     static_cast<CellIndexScalar>(map_idx.y()));

      *cell = CellType(static_cast<Scalar>(0.0), idx);

      m_queue_.emplace(CellSearchQueueTypeCell(cell, cell_idx, cell_idx));
    }

    compute(pc);
  }  // void DistanceMapSparseStatic_::setGoals(const PointCloudType&, Scalar,
 ...
 protected:
  ...
  template <typename PointCloudType>
  void compute(const PointCloudType&)
  {
    ...
    while (!m_queue_.empty())
    {
      ...
      CellIndex idx_current = m_queue_.top().m_cell_idx;
      CellIndex idx_parent = m_queue_.top().m_parent_idx;
      ...
      int k = findNeighbors(neighbors, idx_current.m_row, idx_current.m_col);

      for (int idx = 0; idx < k; idx++)
      {
        CellType children =
            (*m_map_).coeff(neighbors[idx].m_row, neighbors[idx].m_col);

        int r = neighbors[idx].m_row;
        int c = neighbors[idx].m_col;
        int dr = r - idx_parent.m_row;
        int dc = c - idx_parent.m_col;
        ...
        {
          CellType* children_ptr =
              &(*m_map_).coeffRef(neighbors[idx].m_row, neighbors[idx].m_col);

          children_ptr->m_nn_idx = current->m_nn_idx;
          children_ptr->m_dist = dist;

          m_queue_.emplace(CellSearchQueueTypeCell(children_ptr, neighbors[idx],
                                                   idx_parent));
        }
      }
    }

    m_map_->rehash(m_map_->size());

    computeDistDx();
    computeDistDy();
    computeDistD2();
  }  // void DistanceMapSparseStatic_::compute(const PointCloudType&)
  ...
  void computeDistDx()
  {
    for (auto& it : *m_map_)
    {
      ...
      if ((m_map_->rows() > 1) && (r == 0))
      {
        cell.m_dist_d(0) =
            (*m_map_).coeff(1, c).m_dist - (*m_map_).coeff(0, c).m_dist;
        cell.m_dist_d(0) /= m_res_;
        continue;
      }
      else if ((m_map_->rows() > 1) && (r == m_map_->rows() - 1))
      {
        cell.m_dist_d(0) = (*m_map_).coeff(m_map_->rows() - 1, c).m_dist -
                           (*m_map_).coeff(m_map_->rows() - 2, c).m_dist;
        cell.m_dist_d(0) /= m_res_;
        continue;
      }
      else if (m_map_->rows() <= 1)
      {
        cell.m_dist_d(0) = 0;
        continue;
      }
      ...
      cell.m_dist_d(0) =
          (*m_map_).coeff(r + 1, c).m_dist - (*m_map_).coeff(r - 1, c).m_dist;
      cell.m_dist_d(0) *= 0.5 / m_res_;
    }
  }  // void DistanceMapSparseStatic_::computeDistDx()
  ...
  void computeDistDy()
  {
    for (auto& it : *m_map_)
    {
      ...
      if ((m_map_->cols() > 1) && (c == 0))
      {
        cell.m_dist_d(1) =
            (*m_map_).coeff(r, 1).m_dist - (*m_map_).coeff(r, 0).m_dist;
        cell.m_dist_d(1) /= m_res_;
        continue;
      }
      else if ((m_map_->cols() > 1) && (c == m_map_->cols() - 1))
      {
        cell.m_dist_d(1) = (*m_map_).coeff(r, m_map_->cols() - 1).m_dist -
                           (*m_map_).coeff(r, m_map_->cols() - 2).m_dist;
        cell.m_dist_d(1) /= m_res_;
        continue;
      }
      else if (m_map_->cols() <= 1)
      {
        cell.m_dist_d(1) = 0;
        continue;
      }
      ...
      cell.m_dist_d(1) =
          (*m_map_).coeff(r, c + 1).m_dist - (*m_map_).coeff(r, c - 1).m_dist;
      cell.m_dist_d(1) *= 0.5 / m_res_;
    }
  }  // void DistanceMapSparseStatic_::computeDistDy()
  ...
  void computeDistD2()
  {
    for (auto& it : *m_map_)
    {
      ...
      if (m_map_->isOnBorder(idx))
      {
        cell.m_dist_d2.setZero();
        continue;
      }

      CellVectorType dist_d_dxdiff_1 = (*m_map_).coeff(r + 1, c).m_dist_d;
      CellVectorType dist_d_dxdiff_0 = (*m_map_).coeff(r - 1, c).m_dist_d;
      CellVectorType dist_d_dydiff_1 = (*m_map_).coeff(r, c + 1).m_dist_d;
      CellVectorType dist_d_dydiff_0 = (*m_map_).coeff(r, c - 1).m_dist_d;
  ...
  }  // void DistanceMapSparseStatic_::computeDistD2()
 ...
 public:
  ...
  void toImageIdx(RGBImage& img) const
  {
    ...
    for (int r = 0; r < m_map_->cols(); r++)
    {
      ...
      {
        if ((*m_map_).coeff(c, r).m_nn_idx >= 0)
        {
          num_pts_set.emplace((*m_map_).coeff(c, r).m_nn_idx);
        }
      }
    }
    ...
    for (int r = 0; r < m_map_->cols(); r++)
    {
      for (int c = 0; c < m_map_->rows(); c++)
      {
        int idx = (*m_map_).coeff(c, r).m_nn_idx + 1;
  ...
  }  // void DistanceMapSparseStatic_::toImageIdx(RGBImage&) const

  void toImageCellsSet(RGBImage& img) const
  {
    img.create(m_map_->cols(), m_map_->rows());
    img = cv::Vec3b(0, 0, 0);
    ...
    for (int r = 0; r < m_map_->cols(); r++)
    {
      for (int c = 0; c < m_map_->rows(); c++)
      {
        if (m_map_->find(CellIndex(c, r)) != m_map_->end())
        {
          img.at<cv::Vec3b>(r, c) = cv::Vec3b(0, 255, 0);
        }
      }
    }
  }  // void DistanceMapSparseStatic_::toImageCellsSet(RGBImage&) const
                  ...
                  const cv::Scalar& color, int radius = 3) const
  {
    ...
    for (size_t idx = 0; idx < pc.size(); idx++)
    {
     ...
     // DistanceMapSparseStatic_::drawPoints<DrawPointCloudType_>(RGBImage&,
                            ...
                            const cv::Scalar& color, int radius = 3) const
  {
    ...
    for (size_t idx = 0; idx < pc.size(); idx++)
    {
     ...
     // DistanceMapSparseStatic_::drawPointsFromMapIdx<DrawPointCloudType_>(RGBImage&,
     // const DrawPointCloudType_&, const cv::Scalar&, int = 3) const
  ...
  }  // void DistanceMapSparseStatic_::drawPx<DrawPointCloudType_>(RGBImage&,
                        ...
                        const cv::Scalar& color) const
  {
    ...
    for (size_t idx = 0; idx < pc.size(); idx++)
    {
     ...
     // DistanceMapSparseStatic_::drawPxFromMapIdx<DrawPointCloudType_>(RGBImage&,
     // const DrawPointCloudType_&, const cv::Scalar&) const
  ...
  void toImage(RGBImage& img, Channel channel, const PointCloudType&)
  {
    ...
    auto get_channel_value = [this](int r, int c, Channel channel) -> Scalar
    {
      switch (channel)
      {
        ...
        {
          return (*this->m_map_).coeff(r, c).m_dist_d(0);
          break;
        }
        case Channel::kDistDy:
        {
          return (*this->m_map_).coeff(r, c).m_dist_d(1);
          break;
        }
        ...
        default:
        {
          return (*this->m_map_).coeff(r, c).m_dist;
          break;
        }
      }
    };
    ...
    for (int r = 0; r < m_map_->cols(); r++)
    {
      for (int c = 0; c < m_map_->rows(); c++)
      {
        const CellType& cell = (*m_map_).coeff(c, r);
  ...
  void toImage(RGBImage& img, Channel channel, const PointCloudType&)
  {
    ...
    auto get_channel_value = [this](int r, int c, Channel channel) -> Scalar
    {
      switch (channel)
      {
        ...
        case Channel::kDistD2Dxx:
        {
          return (*this->m_map_).coeff(r, c).m_dist_d2(0, 0);
          break;
        }
        case Channel::kDistD2Dxy:
        {
          return (*this->m_map_).coeff(r, c).m_dist_d2(0, 1);
          break;
        }
        case Channel::kDistD2Dyx:
        {
          return (*this->m_map_).coeff(r, c).m_dist_d2(1, 0);
          break;
        }
        case Channel::kDistD2Dyy:
        {
          return (*this->m_map_).coeff(r, c).m_dist_d2(1, 1);
          break;
        }
        default:
        {
 ...
 public:
  ...
  {
    return "DistanceMapSparseStatic "
           "(sparse matrix std::unordered_map, lookup table with NN point "
           "parent idx, distance and 1st derivative)";
  }
  ...
  static inline const char* getTypeName()
  {
    return "DistanceMapSparseStaticWith2ndDeriv "
           "(sparse matrix std::unordered_map, lookup table with NN point "
           "parent idx, distance and 1st+2nd derivatives)";
  }
  ...
  inline bool getNN(CellNNIdxType& nn, const PointVectorType& pt) const
  {
    ...
    assert(m_map_ && "DistanceMapSparseStatic_::getNN | map not created...");
    ...
    const CellType& cell = (*m_map_).coeff(map_idx.x(), map_idx.y());
  ...
  inline bool getNN(CellNNIdxType& nn, const Eigen::Vector2i& pt) const
  {
    ...
    const CellType& cell = (*m_map_).coeff(pt.x(), pt.y());
                       ...
                       const PointCloudType&) const
  {
    ...
    assert(m_map_ && "DistanceMapSparseStatic_::getValue | map not created...");
    ...
    value.m_nn_idx = cell.m_nn_idx;
    value.m_dist = cell.m_dist;
    value.m_dist_d = cell.m_dist_d;

    return true;
  }  // inline bool DistanceMapSparseStatic_::getValue<T, PointCloudType>(T&,
                                   ...
                                   const PointCloudType&) const
  {
    ...
    assert(
        m_map_ &&
        "DistanceMapSparseStatic_::getValueWith2ndDeriv | map not created...");
    ...
    CellVectorType dist_d_dxdiff_1 =
        (*m_map_).coeff(map_idx.x() + 1, map_idx.y()).m_dist_d;
    CellVectorType dist_d_dxdiff_0 =
        (*m_map_).coeff(map_idx.x() - 1, map_idx.y()).m_dist_d;
    CellVectorType dist_d_dydiff_1 =
        (*m_map_).coeff(map_idx.x(), map_idx.y() + 1).m_dist_d;
                                   ...
                                   const PointCloudType&) const
  {
    ...
    CellVectorType dist_d_dxdiff_1 =
        (*m_map_).coeff(pt.x() + 1, pt.y()).m_dist_d;
    CellVectorType dist_d_dxdiff_0 =
        (*m_map_).coeff(pt.x() - 1, pt.y()).m_dist_d;
    CellVectorType dist_d_dydiff_1 =
        (*m_map_).coeff(pt.x(), pt.y() + 1).m_dist_d;
    CellVectorType dist_d_dydiff_0 =
        (*m_map_).coeff(pt.x(), pt.y() - 1).m_dist_d;
                            ...
                            const PointCloudType&) const
  {
    ...
    assert(m_map_ &&
           "DistanceMapSparseStatic_::getValueSubPx | map not created...");
    ...
    const CellType& original_cell = (*m_map_).coeff(
        static_cast<int>(map_pt.x()), static_cast<int>(map_pt.y()));

    if (original_cell.m_nn_idx < 0)
    {
      return false;
    }

    value.m_nn_idx = original_cell.m_nn_idx;
    value.m_dist = cell.m_dist;
    value.m_dist_d = cell.m_dist_d;

    return true;
  }  // inline bool DistanceMapSparseStatic_::getValueSubPx<T,
     // PointCloudType>(T&, const PointVectorType&, const PointCloudType&) const
                                        ...
                                        const PointCloudType&) const
  {
    ...
    assert(m_map_ &&
           "DistanceMapSparseStatic_::getValueSubPxWith2ndDeriv | "
           "map not created...");
    ...
    const CellType& original_cell = (*m_map_).coeff(x0, y0);
    ...
    dist_d_dxdiff_1 = (*m_map_).coeff(x0 + 1, y0).m_dist_d;
    dist_d_dxdiff_0 = (*m_map_).coeff(x0 - 1, y0).m_dist_d;
    dist_d_dydiff_1 = (*m_map_).coeff(x0, y0 + 1).m_dist_d;
    dist_d_dydiff_0 = (*m_map_).coeff(x0, y0 - 1).m_dist_d;
    ...
    dist_d_dxdiff_1 = (*m_map_).coeff(x1 + 1, y0).m_dist_d;
    dist_d_dxdiff_0 = (*m_map_).coeff(x1 - 1, y0).m_dist_d;
    dist_d_dydiff_1 = (*m_map_).coeff(x1, y0 + 1).m_dist_d;
    dist_d_dydiff_0 = (*m_map_).coeff(x1, y0 - 1).m_dist_d;
    ...
    dist_d_dxdiff_1 = (*m_map_).coeff(x0 + 1, y1).m_dist_d;
    dist_d_dxdiff_0 = (*m_map_).coeff(x0 - 1, y1).m_dist_d;
    dist_d_dydiff_1 = (*m_map_).coeff(x0, y1 + 1).m_dist_d;
    dist_d_dydiff_0 = (*m_map_).coeff(x0, y1 - 1).m_dist_d;
    ...
    dist_d_dxdiff_1 = (*m_map_).coeff(x1 + 1, y1).m_dist_d;
    dist_d_dxdiff_0 = (*m_map_).coeff(x1 - 1, y1).m_dist_d;
    dist_d_dydiff_1 = (*m_map_).coeff(x1, y1 + 1).m_dist_d;
    dist_d_dydiff_0 = (*m_map_).coeff(x1, y1 - 1).m_dist_d;
    ...
    return true;
     ...
     // DistanceMapSparseStatic_::getValueSubPxWith2ndDeriv<PointCloudType>(
                                        ...
                                        const PointCloudType&) const
  {
    ...
    assert(m_map_ &&
           "DistanceMapSparseStatic_::getValueSubPxWith2ndDeriv | map not "
           "created...");
    ...
    if ((*m_map_)
            .coeff(static_cast<int>(map_pt.x()), static_cast<int>(map_pt.y()))
            .m_nn_idx < 0)
    {
      return false;
    }
    ...
    return true;
     ...
     // DistanceMapSparseStatic_::getValueSubPxWith2ndDeriv<PointCloudType>(
     // CellTypeWith2ndDeriv&, const PointVectorType&, const PointCloudType& pc)
     // const
 ...
 public:
  ...
  }  // void DistanceMapSparseStatic_::serialize(srrg2_core::ObjectData&,
  ...
  }  // void DistanceMapSparseStatic_::deserialize(srrg2_core::ObjectData&,
     // srrg2_core::IdContext&) override
 ...
 public:
   ...
   * post  : https://stackoverflow.com/q/25375202
   *
  ...
  size_t size() const
  {
    ...
    distance_map_size += (sizeof(CellType) + sizeof(void*)) * m_map_->size();
    distance_map_size +=
        (sizeof(size_t) + sizeof(void*)) * m_map_->bucket_count();
    distance_map_size += sizeof(CellSearchQueueTypeCell) * m_queue_.size();

    return distance_map_size;
  }

};  // class DistanceMapSparseStatic_<CellType_> : public
    // srrg2_core::Serializable

using DistanceMapSparseStaticFloat =
    DistanceMapSparseStatic_<DistanceMapCellFloat>;
using DistanceMapSparseStaticDouble =
    DistanceMapSparseStatic_<DistanceMapCellDouble>;

using DistanceMapSparseStaticWith2ndDerivFloat =
    DistanceMapSparseStatic_<DistanceMapWith2ndDerivCellFloat>;
using DistanceMapSparseStaticWith2ndDerivDouble =
    DistanceMapSparseStatic_<DistanceMapWith2ndDerivCellDouble>;

using DistanceMapSparseStaticFloatPtr =
    std::shared_ptr<DistanceMapSparseStaticFloat>;
using DistanceMapSparseStaticDoublePtr =
    std::shared_ptr<DistanceMapSparseStaticDouble>;

using DistanceMapSparseStaticWith2ndDerivFloatPtr =
    std::shared_ptr<DistanceMapSparseStaticWith2ndDerivFloat>;
using DistanceMapSparseStaticWith2ndDerivDoublePtr =
    std::shared_ptr<DistanceMapSparseStaticWith2ndDerivDouble>;
